function checkCompanyFields(e) {
  var fields = document.getElementsByClassName('required-field');

  for (var i = 0; i < fields.length; i++) {
    element = fields[i]
    var elementId = fields[i].id;
    if (!element.value) {
      $(`#${elementId}`).parent().append('<p class="bg-danger required-error text-center">Please fill this field</p>')
    }
  }
