...
RSpec.describe "db:seed_test_fixture" do
  ...
  xit "seeds the test database from the fixture data" do
    ...
    expect(Company.first.descriptoin).to eq("Uses stem cells derived from the bone marrow of equine and canine to treat equine athlete joint injuries.")
    ...
    expect(Company.last.descriptoin).to eq("Zeoponic soil amendment/fertilizer, NASA spin off, high efficiency ion exchange nutrient delivery; horticulture, sports/golf, forestry, agriculture, landscaping, greenroofs, aquaponics, reclamation.")
