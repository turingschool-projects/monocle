...
RSpec.describe "db:seed_test_fixture" do
  ...
  it "seeds the test database from the fixture data" do
    ...
    expect(Company.first.city).to eq("Fort Collins")
    expect(Company.first.state).to eq("CO")
    expect(Company.first.zip_code).to eq("80524")
    ...
    expect(Company.last.city).to eq("Boulder")
    expect(Company.last.state).to eq("CO")
    expect(Company.last.zip_code).to eq("80308")
  end
end
